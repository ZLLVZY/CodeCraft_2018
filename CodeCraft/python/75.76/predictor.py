...
def get_prdata(input_lines):            #get predict_data from input
    ...
    for item_input in input_lines:      #get predict_flavor
        if item_input[0]=='f':
            ...
            createVar["f%s" % j]=flavor(temp[0],int(temp[1]),int(temp[2])/1024)
...
def train(train_data,interval_num):                 #Adagrad train model:   y=w1x1+w2x2+w3x3+b
    j=interval_num-3
    lr=0.0001                                                        #learning_rate
    train_num=10000                                      #trian_num
    w1,w2,w3,b=0.5,0.5,0.5,0.5                                   #weight&bias
    lr_w1,lr_w2,lr_w3,lr_b=0,0,0,0
    ...
    for i in range(train_num):                          #start train
        w1_grad,w2_grad,w3_grad,b_grad=0,0,0,0
        for k in range(j):
            w1_grad=w1_grad-2*train_data[k]*(train_data[k+3]-w1*train_data[k]-w2*train_data[k+1]-w3*train_data[k+2]-b)
            w2_grad=w2_grad-2*train_data[k+1]*(train_data[k+3]-w1*train_data[k]-w2*train_data[k+1]-w3*train_data[k+2]-b)
            w3_grad=w3_grad-2*train_data[k+2]*(train_data[k+3]-w1*train_data[k]-w2*train_data[k+1]-w3*train_data[k+2]-b)
            b_grad=b_grad-2*(train_data[k+3]-w1*train_data[k]-w2*train_data[k+1]-w3*train_data[k+2]-b)
        ...
        w3=w3-w3_grad*lr/(lr_w3**0.5)
        b=b-b_grad*lr/(lr_b**0.5)
        
    y=round(w1*train_data[-3]+w2*train_data[-2]+w3*train_data[-1]+b)           
    y=int(y)
    if y<0:
        y=0
    return y
...
def predict_vm(ecs_lines, input_lines):
    ...
    T,F,S=get_prdata(input_lines)    #T:pr_time   F:flavor,cpu,mem  S:server's cpu,men
