...
def get_prdata(input_lines):            #get predict_data from input
    ...
    for item_input in input_lines:      #get predict_flavor
        ...
        if item_input[0]=='M':
            d='MEM'
    T=[input_lines[6+int(input_lines[2].strip( ))][0:19],input_lines[7+int(input_lines[2].strip( ))][0:19]]     
...
def train(train_data,interval_num):                 #Adagrad train model:   y=w1x1+w2x2+b
    ...
    w1,w2,b=0.45,0.7,0.5                                 #weight&bias
...
def feature2(N):
    for i in range(len(N)):
        for k in range(len(N)-2):
            ...
            if (N[i][k+1]-N[i][k]<5)and(N[i][k+1]-N[i][k+2]<5):
                N[i][k+1]+=4
            if (N[i][k+1]-N[i][k]>7)and(N[i][k+1]-N[i][k+2]>7):
                N[i][k+1]-=6
            if (N[i][k+1]-N[i][k]<7)and(N[i][k+1]-N[i][k+2]<7):
                N[i][k+1]+=6
    return N
...
def feature3(N):
    ...
    for i in range(len(N)):
        paixu=sorted(N[i])
        if len(N[i])%2!=0:
            q2index=int((len(N[i])-1)/2)  #3
            if q2index%2!=0:
                q2=paixu[q2index]
                q1=(paixu[int((q2index-1)/2)]+paixu[int((q2index+1)/2)])/2
                q3=(paixu[q2index+int((q2index-1)/2)]+paixu[q2index+int((q2index+1)/2)])/2
                jimin=q1-k*(q3-q1)
                jimax=q3+k*(q3-q1)
            else:
                q2=paixu[q2index]
                q1=paixu[int(q2index/2)]
                q3=paixu[q2index+int(q2index/2)]  
                jimin=q1-k*(q3-q1)
                jimax=q3+k*(q3-q1)        
        if len(N[i])%2==0:
            q2index1=int((len(N[i])-2)/2)
            q2index2=int(len(N[i])/2)
            if q2index1%2==0:
                q2=(paixu[q2index1]+paixu[q2index2])/2
                q1=paixu[int(q2index1/2)]
                q3=paixu[q2index2+int(q2index1/2)]
                jimin=q1-k*(q3-q1)
                jimax=q3+k*(q3-q1) 
            else:
                q2=(paixu[q2index1]+paixu[q2index2])/2
                q1=(paixu[int((q2index1-1)/2)]+paixu[int(q2index2/2)])/2
                q3=(paixu[q2index2+int((q2index1-1)/2)]+paixu[q2index2+int(q2index2/2)])/2
                jimin=q1-k*(q3-q1)
                jimax=q3+k*(q3-q1)    
